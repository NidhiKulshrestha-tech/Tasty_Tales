  ...
  const SaveData = async () => {
    ...
    if (Name === '' || Mobile === '' || Email === '' || Password === '') {
      ...
      if (emailerror === '') {
        ...
        console.log('doctorParam', params);
        ...
        const encryptedHashData = await RSA.encrypt(hashData, publicKey);
        console.log('encryptedHashData', encryptedHashData);
  
        const encryptedKeyData = cryptoComponent.encryptAESData(
          privateKey,
          encryptionKey,
        );
        ...
        const encParams = {
          ...
          addInfo: {
            ...
            kID: encryptedKeyData, //sending the encrypted Private key
            guid: deviceId,
          },
        };
        console.log('encParams', encParams);
          ...
          // Converted the data into a single string
          const dataString = JSON.stringify(data.addInfo);
   ...
   await AsyncStorage.setItem('guid', guid);
        ...
        //  Encrypted data
        const EncData = c.encryptAESData(dataString, guid);
        ...
        await AsyncStorage.setItem('EncryptedData', EncData);

        // Hash of the datastring
        const HashData = await sha256(dataString);
        console.log('HASH CODE', HashData);
        await AsyncStorage.setItem('hashkey', HashData);

        // Encrypted HASH
        const encHash = CryptoComponent.encryptAESData(HashData, publicKey);
        console.log('ENC HASH CODE', HashData);
        await AsyncStorage.setItem('EncHash', encHash);

        // Encrypted Private Key
        const KID = CryptoComponent.encryptAESData(privateKey, guid);
        console.log('Encrypted Private Key ', KID);
        await AsyncStorage.setItem('EncKid', KID);

        // Decrypted Private Key
        const DecryptKID = CryptoComponent.decryptAESData(KID, guid);
        console.log('Decrypt Kid', DecryptKID);
        await AsyncStorage.setItem('DecKid', DecryptKID);
        ...
        console.log('encdataStringdataString', encdata);
